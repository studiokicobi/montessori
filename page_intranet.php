...
?>
    	...
    	<?php while ( have_rows( 'medlemslista' ) ) : the_row();
			...
			// Print Child's name
        	echo '<dt class="barn">';
				if ( $child_1 ) : echo $child_1; endif;
            	if ( $child_2 ) : echo '<br>' . '<span class="child-plus">' . $child_2 . '</span>'; endif;
            	if ( $child_3 ) : echo '<br>' . '<span class="child-plus-plus">' . $child_3 . '</span>'; endif;

			echo '</dt>';

				echo '<dd>';
		            // Parent 1 name
		            if ( $first_name_1 ) : echo '<span class="parent">' . $first_name_1 . ' ' . $last_name_1 . '</span>'; endif;
	
    		        // If parent 2 name exists, print tel + email links
    		        if ( $first_name_1 && $tel_1 ) : echo '<a href="tel:' . $tel_1 . '" class="tel-link dashicons-before dashicons-phone">' . $tel_1 . '</a>'; endif;
        		    if ( $first_name_1 && $email_1 ) : echo '<a href="mailto:' . $email_1 . '" class="email-link dashicons-before dashicons-email-alt">' . $email_1 . '</a>'; endif;

					// If parent 2, add break
					if ( $first_name_2 ) : echo '<br>'; endif;

					// If parent 2, print parent 2 name
					if ( $first_name_2 ) : echo '<span class="parent">' . $first_name_2 . ' ' . $last_name_2 . '</span>'; endif;

					// If parent 2 name exists, print tel + email links
					if ( $first_name_2 && $tel_2 ) : echo '<a href="tel:' . $tel_2 . '" class="tel-link dashicons-before dashicons-phone">' . $tel_2 . '</a>'; endif;
					if ( $first_name_2 && $email_2 ) : echo '<a href="mailto:' . $email_2 . '" class="email-link dashicons-before dashicons-email-alt">' . $email_2 . '</a>'; endif;
